...
regions = []
with open('../data/PW_RBSC_6th_cortex.txt','r') as f:
    for line in f.readlines():
        regions.append(int(line.split('\t')[0]))
...
newvalscopy[np.where(~np.isin(newvalscopy,sscx))]=11
newvalscopy[np.where(np.isin(newvalscopy,sscx))]=11 # Sets  Sscx to 1

newvalscopy[np.where(~np.isin(im.numpy(),regions))]=5
...
ants.image_write(newerimage,'../intermediateFiles/paxLabelsAlignedToOsparcMask_Ctx.nii.gz') # Mask of osparc rat
...
newvals[np.where(~np.isin(newvals,sscx))]=11
newvals[np.where(np.isin(newvals,sscx))]=11 # Sets sscx to 1
newvals[np.where(~np.isin(im.numpy(),regions))]=5
newvals[np.where(newvalscopy==0)]=0

newimage = im.new_image_like(newvals)

ants.image_write(newimage,'../intermediateFiles/PWMaskMask_Ctx.nii.gz') # Mask of paxinos-watson atlas
